...
export function updateProgressBar(event) {
    ...
    musicState.audio.currentTime = (widthPercentage * musicState.audio.duration) / 100;
  }
  ...
  export function highlightCurrentSong() {
    document.querySelectorAll(".song").forEach((element, index) => {
      element.style.fontWeight = index === musicState.songCount ? "bold" : "normal";
    });
  }
  ...
  export function updateAudioTime() {
    ...
    const duration = isNaN(musicState.audio.duration) ? "00:00" : formatTime(musicState.audio.duration);
